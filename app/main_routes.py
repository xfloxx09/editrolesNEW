...
def coaching_dashboard():
    ...
    # NEW: If user is a team member (has permission view_own_coachings), restrict to their own coachings
    if current_user.has_permission('view_own_coachings'):
        team_member = current_user.team_member
        if not team_member:
            flash("Ihr Benutzerkonto ist keinem Teammitglied zugeordnet.", "danger")
            return redirect(url_for('main.index'))
        global_base = global_base.filter(Coaching.team_member_id == team_member.id)
    ...
    # Apply the same filter for team members
    if current_user.has_permission('view_own_coachings'):
        team_member = current_user.team_member
        if team_member:
            list_q = list_q.filter(Coaching.team_member_id == team_member.id)
